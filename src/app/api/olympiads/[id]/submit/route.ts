...
export async function POST(
  ...
  try {
    ...
    if (
      ...
      olympiad.questionsPerParticipant < olympiadQuestions.length
    ) {
      // Filter questions to only those that were answered
      olympiadQuestions = olympiadQuestions.filter((q: any) =>
    ...
    for (const question of olympiadQuestions) {
      ...
      if (question.type === 'multiple_choice') {
        ...
        try {
          ...
          console.error(
            'Error evaluating matching answers for question',
            question.id,
            ':',
            error
          );
